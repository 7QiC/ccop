...
// DecodeAttention kernel 声明（接口已定，函数体待实现）。
// 模板参数：T（q/k_cache/v_cache/out 的 dtype，BF16 或 FP32）。
// 第一阶段（正确性为先，scalar 版：每线程完整处理一个 (b, qh) 输出行）。
//
...
//
// 数据布局（全部行主序连续）：
//   q/out: [batch_size, num_q_heads, head_dim]，
//       线性偏移 = (b * num_q_heads + qh) * head_dim + d；
...
//
// 并行映射：
//   grid.x 与 block 内 threadIdx.x 一起平铺 batch_size * num_q_heads 个
//   输出行（block 向上取整，需越界守卫）；
//   blockIdx.x/threadIdx.x → 行索引 rid → b = rid / num_q_heads、
//   qh = rid % num_q_heads；每线程对本请求循环全部 context_lens[b] 个
//   KV token（KV 长度按请求变长，从 context_lens 读取）。
//
// 数值注意：
//   - BF16 参与计算前提升到 float；点积与加权累加用 float 精度；
//   - softmax 数值稳定：exp 前必须减行内 max（score 可能为大正数）；
//     score 无静态存储可放（KV 长度运行时决定），各遍需要时按
//     (i, r) 定位物理块重新计算该元素（或 online-softmax，任选）；
//   - block_table 前 ceil(context_lens[b] / block_size) 个块的有效性由
//     调用方保证（其余对齐行内容任意），kernel 不守卫；
//   - 维度与索引统一 unsigned int（uint32 上限约 42.9 亿，覆盖
//     32 头 × 100 万上下文 × head_dim 的目标场景）。
template <typename T>
__global__ void decode_attention_kernel(const T* q, const T* k_cache, const T* v_cache,
                                        const int32_t* block_table, const int32_t* context_lens,
                                        T* out, float scale, unsigned int batch_size,
    ...
    if (tid >= batch_size * num_q_heads) {
        return;
    }
    unsigned int b = tid / num_q_heads;
    unsigned int qh = tid % num_q_heads;
    unsigned int kvh = qh * num_kv_heads / num_q_heads;
    unsigned int len = context_lens[b];
    float m = -INFINITY;
    ...
    for (unsigned int s = 0; s < len; ++s) {
        ...
        unsigned int r = s % block_size;
        unsigned int blk = block_table[max_blocks_per_req * b + i];
        l += expf(cal_score<T>(q + (b * num_q_heads + qh) * head_dim,
    ...
    for (unsigned int d = 0; d < head_dim; ++d) {
        ...
        out[(b * num_q_heads + qh) * head_dim + d] = static_cast<T>(sum);
    }
}
...
void decode_attention(const Tensor& q, const Tensor& k_cache, const Tensor& v_cache,
    ...
    //     kernel 参数类型（block_table/context_lens 为 const int32_t*）；
    //     模板参数 T 按 dtype 取 __nv_bfloat16（kBFloat16）或 float
    //     （kFloat32）；标量按签名顺序传（scale, batch_size, num_q_heads,
    //     num_kv_heads, head_dim, block_size, max_blocks_per_req）。
    const unsigned int block = 256;
    const unsigned int grid = (batch_size * num_q_heads - 1 + block) / block;
    cudaStream_t s = static_cast<cudaStream_t>(ctx.stream);
    switch (q.dtype()) {
        case DType::kBFloat16:
            decode_attention_kernel<__nv_bfloat16><<<grid, block, 0, s>>>(
