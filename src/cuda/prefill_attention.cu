...
void prefill_attention(const Tensor& q, const Tensor& k_cache, const Tensor& v_cache,
    ...
    assert(total_q_tokens > 0 && num_q_heads > 0 && num_kv_heads > 0 && head_dim > 0);
    assert(block_size > 0);
    ...
    assert(out->shape(0) == static_cast<std::int64_t>(total_q_tokens));
    ...
    assert(query_start_loc.valid());
    assert(query_start_loc.rank() == 1 && query_start_loc.is_contiguous());
    assert(query_start_loc.dtype() == DType::kInt32);
    assert(query_start_loc.shape(0) == static_cast<std::int64_t>(batch_size + 1));
